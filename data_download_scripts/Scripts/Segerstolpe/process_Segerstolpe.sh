...
exec > >(tee -i /work/islet_cartography_scrna/data_download_scripts/Scripts/Segerstolpe/output.log)
exec 2>&1

cd /work/islet_cartography_scrna/data_download_scripts/Scripts/Segerstolpe/
# Internalize shell
...
# Setup the files needed to be downloaded for each cell
    ...
    # Run STAR
    STAR --genomeLoad LoadAndKeep --genomeDir $Genome --soloType SmartSeq --readFilesManifest ./manifest --soloUMIdedup Exact --soloStrand Unstranded --soloFeatures Gene GeneFull --outFilterScoreMin 30 --soloMultiMappers EM --soloCellFilter None --runThreadN 30 --outMultimapperOrder Random --outSAMmultNmax 1 --readFilesCommand zcat
