...
echo "sample, $study_name" > "$tmp/${study_name}_combined.csv"
...
sort "$tmp/${study_name}_combined.csv" -o "$tmp/${study_name}_combined_sorted.csv"

# Transpose
datamash -t, transpose < "$tmp/${study_name}_combined_sorted.csv" > "$out/alignment_qc_${study_name}.csv"
