...
study_number <- base::data.frame(
  study = 1:25,
  name = c("Avrahami", "Baron", "Camunas", "Dai", "Enge", "Fang", "Gurp",
           "HPAP_10x", "HPAP_fluidigm", "HPAP_patch_22", "HPAP_patch_23", 
           "Kang", "Lawlor", "Mauvais_Jarvis", "Motakis", "Muraro", 
           "Segerstolpe", "Shrestha", "Son", "Tritschler", "Wang", 
           "Wang_Sander", "Xin", "Xin_Diabetes", "Zhang")
)
...
camunas_meta <- camunas_facs |>
  ...
  dplyr::mutate(cell_nuclei = "cell",
                islet_center = "Albeta Diabetes Institute IsletCore",
                library_prep = "smart_seq2") |> 
...
# Dai ---------------------------------------------------------------------
dai_gse <- GEOquery::getGEO(filename = here("islet_cartography_scrna/data_raw/meta/Dai/GSE164875_series_matrix.txt"), getGPL = F)
dai_gse <- Biobase::phenoData(dai_gse)@data
load(here::here("islet_cartography_scrna/data_raw/meta/Dai/20230323_pclamp_counts_metadata.RData"))

dai_meta <- patched@meta.data |>
  tibble::rownames_to_column("sample") |> 
  dplyr::select(-orig.ident, -nCount_RNA, -nFeature_RNA, -percent.mt, -Sex)

dai_meta_2 <- dai_gse |>
  dplyr::rename(sample = title,
                disease = `diagnosis:ch1`,
                sex = `Sex:ch1`) |> 
  dplyr::mutate(sample = stringr::str_remove(sample, "pancreatic cells for patch-seq \\["),
                sample = stringr::str_remove(sample, "\\]")) |>
  dplyr::left_join(y = dai_meta, by = "sample") |>
  dplyr::rename_with(~ snakecase::to_snake_case(.x)) |> 
  dplyr::select(-dplyr::contains("data_processing"),
                -dplyr::contains("characteristics_"),
                -dplyr::contains("supplementary"),
  ...
  dplyr::mutate(cell_nuclei = "cell",
                name = "Dai",
                donor_study = base::paste0(name, "_", donor), 
                disease = dplyr::case_when(disease == "Nondiabetic" ~ "nd",
                                           disease == "T2D" ~ "t2d"),
                sex = dplyr::case_when(sex == "Female" ~ "f",
                                           sex == "Male" ~ "m"),
                islet_center = dplyr::case_when(stringr::str_starts(donor, "R") ~ "Alberta Diabetes Institute IsletCore",
                                                stringr::str_starts(donor, "AG") ~ "Allegheny Health Network",
                                                stringr::str_starts(donor, "H") ~ "University of Alberta Clinical Iselt Transplantation Program"),
                library_prep = "smart_seq2") |> 
  dplyr::filter(!is.na(donor)) |>  # remove the 12 cells that does not have a known donor, as we can not use them anyway
  vroom::vroom_write(here::here("islet_cartography_scrna/data/metadata/Dai.csv"), delim = ",", col_names = TRUE)

purrr::map(dai_meta_2, class) |> BiocGenerics::as.data.frame() |> tidyr::pivot_longer(
  cols = dplyr::everything(),
  names_to = "column",
  values_to = "class") |>
  vroom::vroom_write(here::here("islet_cartography_scrna/data/metadata/Dai_class.csv"), delim = ",", col_names = TRUE)
...
kang_meta <- kang |>
  ...
  dplyr::mutate(
    ...
    cell_nuclei = dplyr::case_when(base::grepl("snRNA", sample) ~ "nuclei",
                                   base::grepl("scRNA", sample) ~ "cell"),
...
muraro_meta <- muraro |>
  ...
  dplyr::select(-dplyr::contains("data_processing"),
                ...
                -dplyr::contains("category")) |>
  dplyr::mutate(cell_nuclei = "cell",
                library_prep = "sort_seq|cel_seq2") |> 
...
wang_sander_meta <- wang_sander |> dplyr::filter(molecule_ch1 == "total RNA") |>
  ...
  dplyr::select(-dplyr::contains("data_processing"),
                ...
                -dplyr::contains("supplementary"),
                -dplyr::contains("category")) |>
  dplyr::mutate(cell_nuclei = "nuclei",
                library_prep = "multiome_10x") |> 
