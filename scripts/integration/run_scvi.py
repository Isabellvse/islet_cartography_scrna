...
# Save model (optional)
model_path = os.path.join(model_dir, f'scvi_model_{n}_{key_save}')
...
df_path = os.path.join(file_dir, f"scvi_latent_embd_{n}_{key_save}.csv")
...
scvi_meta = {
"_scvi_uuid": adata.uns.get("_scvi_uuid", None),
"_scvi_manager_uuid": adata.uns.get("_scvi_manager_uuid", None)}

uuid_path=os.path.join(file_dir, f'scvi_{n}_{key_save}_model_uuid.pkl')

with open(uuid_path, "wb") as f:
    pickle.dump(scvi_meta, f)
