...
sample_thresholds <- data.frame("name" = c(rep("Gurp",2), rep("HPAP_10x", 4), rep("Wang_Sander", 4)),
...
thresholds_updated <- purrr::imap(quality_met, function(qc, name) {
  ...
  thresholds_original <- qc |> 
    ...
    not_match <- dplyr::anti_join(x = thresholds_updated, 
...
quality_met_updated <- imap(quality_met, function(df, name){
  df |> 
    dplyr::left_join(y = thresholds_updated[[name]], 
                     relationship = "many-to-one") |> 
...
qc_diff_columns <- purrr::map2(
  ...
  ~ {
    ...
    colnames(.x[diff_columns]) |> stringr::str_remove("threshold_") |> 
      stringr::str_remove("_lower") |> stringr::str_remove("_upper") |> unique()
  }
)
...
quality_met_updated <- purrr::imap(quality_met_updated, function(df, study_name){
  ...
  df_out <- df |> 
    dplyr::mutate(excluded = dplyr::case_when(excluded == "excluded" ~ "excluded",
...
excluded_included_sum <- quality_met_updated |> 
  purrr::modify_depth(1, ~ if (base::unique(.x$platform) %in% c("droplet", "plate_barcode")) {
    .x |> dplyr::group_by(ic_id_sample, excluded, platform) |> 
      dplyr::tally()
  } else if (base::unique(.x$platform) %in% c("plate")) {
    .x |> dplyr::group_by(ic_id_donor, excluded, platform) |> 
      dplyr::tally()
    })
...
purrr::iwalk(excluded_included_sum, function(df, name){
  ...
  if (base::unique(df$platform) %in% c("droplet", "plate_barcode")) {
    plot <- df |> 
      ...
      ggplot2::labs(y = "Number of barcodes",
                    fill = "Status",
                    title = name) +
      my_theme() +
      ggplot2::theme(
