...
markers <- purrr::map(meta_paths, \(df) {
  ...
  purrr::imap(\(df, name) df |> 
                ...
                dplyr::select(gene_symbol, TE, seTE, fdr, lower, upper, k.study, dplyr::all_of(colnames(perc))) |> 
                dplyr::arrange(desc(TE), desc(.data[[name]]), desc(k.study)) |>
                head(4)) |> 
  purrr::list_rbind(names_to = "cell_type") |> 
  dplyr::select(gene_symbol, marker = cell_type, k.study)
...
p1 <- wrap_plots(plot_list, nrow = 1) + patchwork::plot_layout(guides = "collect")


plot_bar <- logfc |> 
  dplyr::filter(cell_type == marker)  |>  
  ...
  purrr::map(\(df){
    ...
    df|>  
      ...
      ggplot2::scale_y_reverse(breaks = scales::breaks_pretty(n = 2))+
      my_theme() +
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.5, hjust = 1),
                     panel.spacing = unit(0.1, "lines"),
                     axis.title = ggplot2::element_blank(),
...
p2 <- wrap_plots(plot_bar, nrow = 1) + patchwork::plot_layout(guides = "collect")


pdf(
  file = paste0(base_path, "/cell_type_between/plots/dotplot_top_4_marker_genes_n_study.pdf"),
  height = 1.5,
  width = 5)
